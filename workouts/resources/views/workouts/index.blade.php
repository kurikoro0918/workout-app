...
<div class="container mx-auto p-4 md:p-8">
    ...
    <header class="text-center mb-10">
        <h1 class="text-4xl md:text-5xl font-bold text-gray-800 mb-2">My Workout Log</h1>
        <p class="text-gray-600">日々の努力を記録し、成果を可視化しよう。</p>
        <div class="mt-4 flex justify-center space-x-4">
            ...
            <a href="{{ route('profile.edit') }}" class="inline-block bg-white hover:bg-gray-50 text-gray-800 font-semibold py-2 px-4 rounded-lg transition-colors duration-200 border border-gray-300">
                プロフィール編集
            </a>
    ...
    <div class="text-center mb-8">
        <a href="{{ route('workouts.create') }}" class="inline-flex items-center justify-center bg-gray-800 hover:bg-gray-700 text-white font-semibold py-3 px-6 rounded-xl transition-colors duration-200 shadow-md">
        ...
        @forelse ($workouts as $workout)
            <div class="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow duration-300 transform hover:-translate-y-1">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-2xl font-bold text-gray-800">{{ $workout->exercise_name }}</h2>
                    <div class="flex space-x-2 opacity-70 hover:opacity-100 transition-opacity">
                        <!-- Edit Button -->
                        <a href="{{ route('workouts.edit', $workout->id) }}" class="text-gray-500 hover:text-blue-500">
                ...
                <p class="text-xs text-gray-500 mt-4">記録日: {{ $workout->created_at->format('Y年m月d日 H:i') }}</p>
            </div>
        @empty
